...
func OpenDbConnection(connectionString string, logSql bool) (*gorm.DB, *pgx.Conn, error) {
	...
	connection, err := getConnection(connectionString)
	if err != nil {
		return nil, nil, err
	}
...
func getConnection(connectionString string) (*pgx.Conn, error) {
	...
	for {
		...
		case <-ticker.C:
			...
			client, err := pgx.Connect(context.Background(), connectionString)
			if err != nil {
				logger.With(zap.Error(err)).Warn("не удалось установить соединение с PostgreSQL")
